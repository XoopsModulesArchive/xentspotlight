<?php
/**
 * $Id: index.php,v 1.4 2005/04/07 15:10:27 m4d3l Exp $
 * Module: Spotlight
 * Version: v2.0
...
function spotlightNewsForm()
{
    ...
    spot_adminmenu(_AM_SPOTLIGHT_NAME_NEWS, 0);

    $sql = $xoopsDB->query('SELECT sid, item, auto, image, auto_image, imagealign FROM ' . $xoopsDB->prefix('xentspotlight') . ' WHERE sid=1');

    [$sid, $item, $auto, $indeximage, $auto_image, $imagealign] = $xoopsDB->fetchRow($sql);
    ...
    $imagealign_select->addOptionArray(['left' => _AM_SPOT_LEFT, 'center' => _AM_SPOT_CENTER, 'right' => _AM_SPOT_RIGHT]);
...
function spotlightWFSectionForm()
{
    ...
    spot_adminmenu(_AM_SPOTLIGHT_NAME_WFSS, 0);

    $sql3 = ('SELECT item, auto, image, auto_image, imagealign FROM ' . $xoopsDB->prefix('xentspotlight') . ' WHERE sid=2;');

    $result3 = $xoopsDB->query($sql3, 0, 1);

    [$item, $auto, $indeximage, $auto_image, $imagealign] = $xoopsDB->fetchRow($result3);
...
switch ($op) {
    case 'submit':
        ...
        $auto = $auto ?? 0;
        $auto_image = $auto_image ?? 0;
        $imagealign = addslashes($_POST['imagealign']);

        $image = ('blank.png' != $_POST['indeximage']) ? $myts->addSlashes($_POST['indeximage']) : '';

        $sql = sprintf('UPDATE ' . $xoopsDB->prefix('xentspotlight') . " SET item='" . $item . "', auto='" . $auto . "', image='" . $image . "', auto_image='" . $auto_image . "', imagealign='" . $imagealign . "' WHERE sid = $idtype");
    ...
    default:
        xoops_cp_header();
        spot_adminmenu(_AM_SPOTLIGHT_NAME_CONF);
        break;
}
xoops_cp_footer();
